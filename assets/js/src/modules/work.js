...
;(function(factory) {
  ...
  $.event.special.tap = (function() {
    ...
    if (!document.addEventListener)
      return { bindType: 'click', delegateType: 'click' }
  ...
  ml.Work = {
    ...
    bindEvents: function() {
      ...
      $('body').on('mlorientationchange', function() {
        if (_this.currentFullVideo) {
          _this.currentFullVideo.pause()
        }
      })
      ...
      $('.work-media .video-bg-container .play-full-screen').mouseover(
        function() {
      ...
      $(document).keydown(function bindKeyDown(e) {
        if (_this.$curWorkItem == null) {
          return
        }

        var curCarouselItem = _this.$curWorkItem.find('.carousel-item')[
          _this.carouselIndex
        ]

        if (e.keyCode == 37) {
          // console.log( "left pressed" );
          _this.navigateCarouselPrevNext(
            $('#work-item-stage .carousel-arrow-nav .prev')
          )
        ...
        if (e.keyCode == 39) {
          ...
          _this.navigateCarouselPrevNext(
            $('#work-item-stage .carousel-arrow-nav .next')
          )
    ...
    setUpWorkPage: function() {
      ...
      this.$workItemsContainer
        .addClass('loaded')
        .css('width', workItemsContainerWidth)
    ...
    openWorkItem: function($item) {
      ...
      if (ML_vars.device != 'mobile') {
        ...
        _this.$workItemStage
          .find('.copy-wrap, .carousel-arrow-nav')
          .addClass('slidein')
    ...
    closeWorkItem: function($item) {
      ...
      if (ML_vars.device != 'mobile') {
        $item.css('width', this.workItemWidth)
    ...
    resetWorkItem: function($item) {
      ...
      $item
        ...
        .addClass('show-slide')
        .siblings()
        .removeClass('show-slide')
      $item
        .find('.carousel-nav li')
        .eq(0)
        .addClass('active')
        .siblings()
        .removeClass('active')
    ...
    buildWorkCarousels: function() {
      ...
      if (ML_vars.device != 'mobile') {
        this.$workItems.each(function() {
          ...
          if (
            $('.carousel-item:first .work-copy .desc-wrap p', $this).length ===
            1
          ) {
            return
          }
          ...
          workCopyOverlay
            .addClass('overlay')
            .appendTo($('.work-carousel .carousel-item:first-child', $this))

          $('.description:first .desc-wrap p:first', $this).after(
    ...
    navigateCarouselPrevNext: function($navButton) {
      ...
      $('.carousel-nav li', $curCarousel)
        .eq(this.carouselIndex)
        .addClass('active')
    ...
    showPrevNextP: function(dir) {
      var $itemContainer =
        ML_vars.device === 'mobile' ? this.$curWorkItem : this.$workItemStage
      ...
      if (
        $itemContainer.find('.carousel-item').length <= 2 &&
        ML_vars.device != 'mobile'
      ) {
        return
      }
    ...
    playFullVideo: function() {
      ...
      if (ML_vars.device === 'mobile') {
        $fullVideo = $('.work-item.open').find('.work-full-video')
        videoSrc = $fullVideo.data('src')
      }
      ...
      $fullVideo.css('opacity', 0).attr('id', 'featured-work-video')

      $fullVideo.show().css('opacity', 1)
    ...
    closeFullVideo: function() {
      var $fullVideo = $('#work-item-stage').find('.work-full-video')
    ...
    resizeWorkPage: function() {
      ...
      if ($('.work-item.open').length > 0) {
        ...
        this.workItemsContainerWidth = containerWidth =
          itemWidth * this.numWorkItems
        $('.scrollableArea').css('width', this.workItemsContainerWidth)
      }
  ...
  ml.Work.menu = {
    ...
    setUpFilter: function($filterLink) {
      ...
      _this.worksInCat =
        _this.filterCat === 'all'
          ? ml.Work.$workItems
          : $('[data-category="' + _this.filterCat + '"]')
      ;(_this.worksToHide = ml.Work.$workItems.not(_this.worksInCat)),
        (_this.workItemsWidth = ml.Work.workItemsContainerWidth =
          this.workItemWidth * _this.worksInCat)
    },
