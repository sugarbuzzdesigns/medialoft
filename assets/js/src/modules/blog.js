  ...
  ml.Blog = {
    ...
    bindEvents: function () {
      ...
      $(".blog-article .header").on("click", function (e) {
        ...
        if ($(this).parent().is(".show-article")) {
          return
        }
    ...
    setUpSocialLinks: function () {
      $(".blog-article .social a, .carousel .share").each(function (i, link) {
        const twitterURI = `http://twitter.com/intent/tweet?status=`

        var parent = $(this).parents("article")
        var url = $(link).data("url")
        var title = parent.find(".header .title").text()

        if ($(link).is(".twitter")) {
          $(link).attr(
            "href",
            `${twitterURI}${encodeURIComponent(title)}${encodeURIComponent(
