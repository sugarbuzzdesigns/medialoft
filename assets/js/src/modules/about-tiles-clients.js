;(function($) {
  ml.About.tileSwap = {
    ...
    showClient: function() {
      ...
      var tileToFill = $('.client.blank')[
        this.getRanNum($('.client.blank').length)
      ]
    ...
    addImage: function(tile, client) {
      ...
      $(tile).append(
        '<div class="hover"><img src="' +
          clientsInfo[client].logo_hover +
          '"/></div>'
      )
  ...
  $(function() {
    if (!isLoggedIn) {
      var filteredClients = clients.filter(function(value, index, arr) {
        return value !== 'United Health' && value !== 'Optum'
      })
      clients = filteredClients
    } else {
      console.log('keep the logos in')
    }
